...
let GitRemoteCommand = class GitRemoteCommand extends command_1.Command {
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!app.repo_url) {
                throw new errors_1.FatalException(`Missing ${chalk.bold('repo_url')} property in app.`);
            }
            if (!(yield fs_1.pathExists(path.join(this.env.project.directory, '.git')))) {
                ...
                yield this.env.shell.run('git', ['init'], { showSpinner: false, cwd: this.env.project.directory });
            }
            ...
            if (found) {
                if (matches) {
                    ...
                    yield this.env.shell.run('git', ['remote', 'set-url', 'ionic', remote], { showSpinner: false, cwd: this.env.project.directory });
                    this.env.log.ok(`Updated remote ${chalk.bold('ionic')}.`);
                }
            }
            else {
                yield this.env.shell.run('git', ['remote', 'add', 'ionic', remote], { showSpinner: false, cwd: this.env.project.directory });
